...
The `framing/`, `retrieval/`, `brief/`, `critique/`, `revision/`, and `evaluation/` stages write canonical **`framing_decision`**, **`retrieval_result`**, **`piece_brief`**, **`critique_result`**, **`revision_result`**, and **`evaluation_result`** JSON (v1 schemas under `schemas/json/`); drafting and final stages still use the generic skeleton envelope until their domain schemas land.
