...
The `framing/`, `retrieval/`, `brief/`, `critique/`, and `revision/` stages write canonical **`framing_decision`**, **`retrieval_result`**, **`piece_brief`**, **`critique_result`**, and **`revision_result`** JSON (v1 schemas under `schemas/json/`); drafting, evaluation, and final stages still use the generic skeleton envelope until their domain schemas land.
