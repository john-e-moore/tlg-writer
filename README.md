...
The `framing/`, `retrieval/`, `brief/`, and `critique/` stages write canonical **`framing_decision`**, **`retrieval_result`**, **`piece_brief`**, and **`critique_result`** JSON (v1 schemas under `schemas/json/`); drafting and downstream stages still use the generic skeleton envelope until their domain schemas land.
