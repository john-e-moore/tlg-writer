...
4. Add batch-run artifact output for those scripts under `artifacts/runs/<run_id>/` with manifests. **Shipped:** each stub batch run writes `manifest.json` validated as `corpus_batch_manifest` (including optional `batch_statistics` v1 on new runs), plus `summary.md` and `logs/run.log` (distinct from editorial `run_manifest` until unified).
...
17. **LLM boundary in Phase 0 runs:** skeleton runners invoke `tlg_writer.llm_client` at least once per run (default `StubLLMClient`) and record a shared **probe** in each stage `metrics.json` (`llm.phase0_client_probe`), `config.json` (`llm_client_probe`), and `logs/run.log`. By default, stage `output.json` documents remain **stub-built** (probe completion text is not consumed as stage output). **Optional (assigned only):** `run_assigned_skeleton` / `scripts/run_assigned_skeleton.py` may set `llm_framing` / `--llm-framing` to run one additional chat completion whose text is parsed as JSON and validated as `framing_decision` (requires a non-stub client, e.g. `TLG_LLM_BACKEND=openai` and `OPENAI_API_KEY`, or an injected `llm_client` in tests). Framing `metrics.json` then records `llm.framing_completion`; `config.json` may include `llm_framing` with requested/effective model ids.
